...
import { DashboardOutlined, CalendarOutlined, BookOutlined, DownOutlined, FileTextOutlined, LineChartOutlined, SettingOutlined, QuestionCircleOutlined,FilterOutlined, PlusOutlined, UnorderedListOutlined, CloseCircleOutlined } from '@ant-design/icons';
...
function App() {  
  ...
  const [cardInformation,setCardInformation] = useState([
    ...
    },
    {
      title:'避開直覺陷阱',
      book_movie:'快思慢想',
      context: '《快思慢想》(Thinking, Fast and Slow)這本書，是卡尼曼過去數十年對經濟學和行為心理學的研究精華。卡尼曼根據他對人類思考的理解，結合過去心理學的研究，提出人的思考可以分為兩個系統。系統一是本能反應的思考，運行速度快，能量消耗少，而且無意識中進行。例如用廣東話說話時我們不用思考某個字要怎樣發音、聲調是甚麼。',
      tags:['personal','growth', 'Goals']
    }
  ])
  ...
  const popUpAction = (title,book_movie,context,cardTags,cardIndex) => {
    setPop(true)
    setTitle(title)
  ...
  // const handleView = (i) => {
  //   const viewNote = cardInformation.filter ((card, index) => index == i);
  //   setCardInformation (viewNote)
